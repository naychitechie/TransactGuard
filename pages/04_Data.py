...
/* Apply stat-label styles to checkbox labels */
...
def create_scatter_plot(df, x_col, y_col, show_trend=False):
    ...
    fig.add_trace(go.Scatter(
        ...
        name='Legitimate',
        hovertemplate=f'<b><span style="color: #94a3b8;">Legitimate</span></b><br>{x_col}: %{{x}}<br>{y_col}: %{{y}}<extra></extra>'
    ))
    fig.add_trace(go.Scatter(
        x=df[fraud_mask][x_col],
        y=df[fraud_mask][y_col],
        ...
        name='Fraudulent',
        hovertemplate=f'<b><span style="color: #94a3b8;">Fraudulent</span></b><br>{x_col}: %{{x}}<br>{y_col}: %{{y}}<extra></extra>'
    ))
...
st.markdown('<div style="margin-bottom: 32px;"><h1 style="color: #e2e8f0; font-size: 36px; font-weight: 800; margin: 0;">Interactive Data Explorer</h1></div>', unsafe_allow_html=True)
...
with col_right:
    ...
    st.markdown('<div class="control-section"><h5>Visualization Type</h5></div>', unsafe_allow_html=True)
